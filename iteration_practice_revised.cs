...
namespace iteration_basic_project
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> duplicateItems = states.GroupBy(x => x).Where(g => g.Count() > 1).Select(g => g.Key).ToList();                      
            foreach (string input in duplicateItems)
            {
                Console.WriteLine(input + " is a duplicate");
            }
